...
def list_to_pq(fork_edges, contrib_edges, data_dir='data'):
    ...
    edges_pd = pd.DataFrame(edges, columns = ['id'])
    vertices_pd = pd.DataFrame(vertices, columns = ['src', 'dst'])
    ...
    pq.write_table(vertices_table, data_dir + 'vertices.parquet')
    pq.write_table(edges_table, data_dir + 'edges.parquet')
